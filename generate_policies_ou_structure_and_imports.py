...
IMPORT_POLICY_TF = "import_policies.tf"
...
"""
        ...
        if attachment_dict.get(cp_name, None) is None:
...
"""
        ...
        with open(IMPORT_POLICY_TF, "w") as f:
            f.write(
                f"""# This file was automatically generated by {os.path.basename(__file__)} and may require manual review
"""
            )
    ...
    for policy_to_query in policy_list:
        ...
        logging.info(f"These are all the {policy_to_query} types: {all_type_policies}")

        if not skip_import_creation:
            with open(IMPORT_POLICY_TF, "a") as f:
                logging.debug("Generating policy import manifest...")
